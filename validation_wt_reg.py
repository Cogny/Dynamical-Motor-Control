...
lossL2 = tf.add_n([tf.nn.l2_loss(v) for v in network_params]) * 0.03
...
for episode in range(np.int(20e3)):
    batch_ind = 0    
...
#%%
plant = plantDynamics('validation',0,arm_states) #'random' for generating random points, 'fixed' for th example start and target points
...
for episode in range(np.int(16)):
    ...
    net_states_episodes.append(np.vstack(net_states))
    observations, inlets,net_states,dXdotdX, dXdotdU = [],[],[],[],[]
...
for episode in range (16):
    colorVal = scalarMap.to_rgba(ind_batch[episode])    
    plt.plot(net_states_episodes[episode][1:,64],color=colorVal)
plt.locator_params(axis='y', nbins=6)
plt.locator_params(axis='x', nbins=6)
plt.xlabel('Time step', fontsize=16)
plt.ylabel('Neural activation', fontsize=16)
plt.hold(False)
#%%
plt.figure()
plt.grid(True)
...
for episode in range (16):
    ...
    colorVal = scalarMap.to_rgba(ind_batch[episode])    
    for t in range(kf):
        J = plant.jacobian(s_episodic_batch[episode,t,0:2])
        v[t] = np.dot(J.dot(s_episodic_batch[episode,t,2:4]),J.dot(s_episodic_batch[episode,t,2:4]))
    plt.plot(v,'-',color=colorVal,linewidth=2)
plt.locator_params(axis='y', nbins=6)
plt.locator_params(axis='x', nbins=6)
plt.xlabel('Time step', fontsize=16)
plt.ylabel('Hand velocity', fontsize=16)
plt.hold(False)